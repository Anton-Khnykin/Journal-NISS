...
import { getCurrentIssue } from 'middleware/api/public';
import { isEmpty } from 'utils/validation';
import { getShortNames, getDate } from 'utils/data_parser';
import { journalName } from 'static/page_content/general';
...
CurrentIssue.getInitialProps = async () => {
...
function CurrentIssue(props) {
  ...
  const title = issue.volume
    ? `Том\xa0${issue.volume.number}\xa0(${issue.volume.year}),\xa0№${issue.number}\xa0(${issue.number_general})`
    : 'Выпуски';
      ...
      <Layout>
        {isEmpty(issue) ?
          <Box
            ...
            pt={4}
          >
            <NothingToDisplayText text="Нет текущего выпуска" />
          </Box> 
          :
          <>
            <Box
